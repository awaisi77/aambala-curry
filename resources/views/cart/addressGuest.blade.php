    ...
    <div class="px-4">
      ...
      <div class="card-content border-top">
        ...
        <div class="form-group{{ $errors->has('addressID') ? ' has-danger' : '' }}">
            @if(count(\App\Address::all()->where('user_id',340605287)) && \Illuminate\Support\Facades\Session::has("session_address_id"))
                <select name="addressID" id="addressID" class="form-control{{ $errors->has('addressID') ? ' is-invalid' : '' }}" required>
                    @foreach (\App\Address::all()->where('id',\Illuminate\Support\Facades\Session::get("session_address_id"))->reverse() as $address)
